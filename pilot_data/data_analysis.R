...
#Data run and normalized in two different ways with either regular normalization or the SCtransform
...
SCOP_hepatocytes <- readRDS(here::here("pilot_data/subset_analysis_hepatocytes.rds"))
...
DimPlot(SCOP_hepatocytes, split.by = "genotype")
Idents(SCOP_hepatocytes)<-"genotype"
DEG_test <- FindMarkers(SCOP_hepatocytes, ident.1 = "HNKO", ident.2 = "WT")
head(DEG_test)
...
goResults <- enrichGO(gene = eg$ENTREZID,
                      ...
                      OrgDb = org.Mm.eg.db,
                      ont = "BP")
#write_rds(goResults, here::here("pilot_data/GOresults_hep.rds"))
goResults <- readRDS(here::here("pilot_data/GOresults_hep.rds"))
...
SCOP <- readRDS(here("pilot_data/SCOP_annotated.rds"))
...
#saveRDS(SCOP.markers, here('pilot_data/SCOP_markers_SCT.rds'))
SCOP.markers<-readRDS(here('pilot_data/SCOP_markers_SCT.rds'))
VlnPlot(SCOP, features = "Sds")
p <- FeaturePlot(SCOP, features = "Cps1", split.by = "HTO_classification", combine = F)
plot_grid(plotlist = p, ncol = 4)
#saveRDS(SCOP, here('pilot_data/SCOP_after_clustering_SCT.rds'))
SCOP <- readRDS(here::here("pilot_data/SCOP_after_clustering_SCT.rds"))
...
#Investigate difference between Leukocyte pops
